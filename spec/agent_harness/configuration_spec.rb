...
RSpec.describe AgentHarness::Configuration do
  ...
  describe "MCP configuration" do
    it "stores canonical MCP servers via the Ruby DSL" do
      ...
      expect(config.mcp_servers.first).to be_a(AgentHarness::McpServer)
      expect(config.mcp_servers.first.command_argv).to eq(["npx", "server"])
    end
    ...
    it "loads MCP servers from a file" do
      ...
      expect(config.mcp_servers.map(&:name)).to eq(["remote-db"])
    end
  end
