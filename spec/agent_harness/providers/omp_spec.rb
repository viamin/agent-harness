...
RSpec.describe AgentHarness::Providers::OhMyPi do
  ...
  describe ".installation_contract" do
    it "exposes Oh My Pi CLI install metadata pinned to 18.2.4" do
      ...
      expect(contract[:package]).to eq("@oh-my-pi/pi-coding-agent@18.2.4")
      expect(contract[:supported_versions]).to eq(["18.2.4"])
      expect(contract[:version_requirement]).to eq(["= 18.2.4"])
      expect(contract[:install_command]).to eq(
        ["npm", "install", "-g", "--ignore-scripts", "@oh-my-pi/pi-coding-agent@18.2.4"]
      )
    end
    ...
    it "includes Bun runtime requirements so consumers can provision Bun first" do
      ...
      expect(bun[:install_command]).to eq(
        ["sh", "-c", "curl -fsSL https://bun.sh/install | BUN_VERSION=1.4.2 bash"]
      )
      expect(bun[:install_command_string]).to eq(
        "curl -fsSL https://bun.sh/install | BUN_VERSION=1.4.2 bash"
      )
    end
    ...
    it "normalizes surrounding whitespace in supported versions" do
      contract = described_class.installation_contract(version: " 18.2.4 ")

      expect(contract[:version]).to eq("18.2.4")
      expect(contract[:install_command]).to eq(
        ["npm", "install", "-g", "--ignore-scripts", "@oh-my-pi/pi-coding-agent@18.2.4"]
      )
    end
  ...
  describe ".bun_runtime_contract" do
    it "exposes a pinned Bun install command and requirement" do
      ...
      expect(bun).to include(
        ...
        source: :script,
        install_script_url: "https://bun.sh/install",
        install_command: ["sh", "-c", "curl -fsSL https://bun.sh/install | BUN_VERSION=1.4.2 bash"],
        install_command_string: "curl -fsSL https://bun.sh/install | BUN_VERSION=1.4.2 bash"
      )
    end
  ...
  describe "instance" do
    ...
    describe "#noisy_error_patterns" do
      it "matches Oh My Pi startup banner noise" do
        ...
        expect(patterns.any? { |p| "Oh My Pi v18.0.8" =~ p }).to be true
    ...
    describe "#smoke_test" do
      ...
      it "classifies model-not-found failure output emitted alongside startup banner noise" do
        ...
        expect(provider.noisy_error_patterns.any? { |p| "Oh My Pi v18.0.8" =~ p }).to be true
      end
    end
