...
RSpec.describe AgentHarness::Providers::Registry do
  ...
  describe "#install_contract" do
    ...
    it "raises a configuration error when an adapter has not opted into install_contract" do
      ...
      }.to raise_error(AgentHarness::ConfigurationError, /does not expose an install contract/)
    end
  end
  ...
  describe "#installation_contract" do
    it "returns install metadata for builtin providers that expose it" do
      ...
      expect(contract[:default_version]).to eq("7.5.5")
    end
    ...
    it "forwards target selection options to the provider" do
      contract = registry.installation_contract(:kilocode, version: "7.5.5")

      expect(contract[:install_command]).to eq(
        ["npm", "install", "-g", "--ignore-scripts", "@kilocode/cli@7.5.5"]
      )
    end

    it "forwards target selection options to providers with generic contracts" do
      contract = registry.installation_contract(:opencode, version: "1.18.19")
  ...
  describe "#installation_contracts" do
    it "returns providers with installation contracts" do
      ...
      expect(contracts[:opencode][:postinstall_command]).to include("\"$binary_path\" --version >/dev/null")
  ...
  describe "#provider_metadata" do
    ...
    it "uses a stable installation metadata shape across builtin providers" do
      ...
      expect(kilocode_installation).to include(
        ...
        resolved_version: "7.5.5",
        supported_version_requirement: "= 7.5.5",
        binary_name: "kilo",
        install_command: ["npm", "install", "-g", "--ignore-scripts", "@kilocode/cli@7.5.5"],
        install_command_string: "npm install -g --ignore-scripts @kilocode/cli@7.5.5"
      )
    end
