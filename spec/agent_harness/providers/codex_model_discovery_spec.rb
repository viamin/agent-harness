...
RSpec.describe "Codex subscription model discovery" do
  ...
  it "does not retry when fresh discovery recommends the rejected model" do
    allow(executor).to receive(:execute).with(["codex", "--version"], timeout: 2, env: env).and_return(version_result)
    expect_model_list(
      env: env,
      models: [
    ...
    expect(recovery[:discovery]).to include(status: :available, recommended_model_id: "gpt-5.4")
  ...
  def expect_model_list(env:, models:)
    ...
    expect(executor).to receive(:execute)
      .with(
        ...
        hash_including(env: env, timeout: 5, stdin_data: include("\"method\":\"model/list\""))
      )
      .and_return(result)
  end
end
