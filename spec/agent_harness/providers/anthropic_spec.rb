  ...
  describe ".install_contract" do
    it "exposes the official install contract" do
      ...
      expect(contract.dig(:install, :command)).to eq(
        "tmp_script=$(mktemp) && trap 'rm -f \"$tmp_script\"' EXIT && curl -fsSL https://claude.ai/install.sh -o \"$tmp_script\" && bash \"$tmp_script\" #{described_class::SUPPORTED_CLI_VERSION}"
      )
      ...
      expect(contract.dig(:supported_versions, :default)).to eq(described_class::SUPPORTED_CLI_VERSION)
      expect(contract.dig(:supported_versions, :requirement)).to eq(described_class::SUPPORTED_CLI_REQUIREMENT.requirements.map { |op, ver| "#{op} #{ver}" }.join(", "))
