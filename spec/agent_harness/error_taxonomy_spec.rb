...
RSpec.describe AgentHarness::ErrorTaxonomy do
  ...
  describe ".classify_message" do
    ...
    it "classifies installation and platform compatibility errors" do
      expect(described_class.classify_message("Dynamic loader not found: /lib64/ld-linux-x86-64.so.2")).to eq(:installation)
      expect(described_class.classify_message("cannot execute binary file: Exec format error")).to eq(:installation)
      expect(described_class.classify_message("Unsupported OpenCode architecture: Linux/riscv64")).to eq(:installation)
    end
  ...
  describe ".action_for" do
    it "returns correct action for each category" do
      ...
      expect(described_class.action_for(:installation)).to eq(:escalate)
  ...
  describe ".retryable?" do
    ...
    it "returns false for non-retryable categories" do
      expect(described_class.retryable?(:installation)).to be false
  ...
  describe ".description_for" do
    it "returns description for known categories" do
      expect(described_class.description_for(:installation)).to eq("Installation or platform compatibility failed")
  ...
  describe ".categories" do
    it "returns all category names" do
      ...
      expect(categories).to include(:installation, :rate_limited, :auth_expired, :transient, :idle_timeout, :permanent, :sandbox_failure)
    end
  end
end
